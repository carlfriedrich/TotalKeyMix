...
configFile := A_Args.Length() ? A_Args[1] : "config.ini"
...
if (osdPosX >= 0) {
    osdBarOptions .= " X" osdPosX
}
if (osdPosY >= 0) {
    osdBarOptions .= " Y" osdPosY
}
...
;=================== Setup GUI ===================
MenuTray(NoStandard)
if (hideTrayIcon = 0 && FileExist("icon.ico")) {
...
ShowSetupGUI() {
    global setupGUIVisible, volumeUpHotkey, volumeDownHotkey, volumeMuteHotkey, oscIP, oscPort, oscAddress, configFile
...
ButtonOK() {
    global setupGUIVisible, volumeUpHotkey, volumeDownHotkey, volumeMuteHotkey, oscIP, oscPort, oscAddress, configFile, socket
    Gui.Submit()
...
CloseApp() {
    global volume, configFile, socket
...
VolumeUp() {
    global muted, volume, volumeBeforeMuted, volumeStep, volumeMaxValue, socket, oscAddress
    if (muted = 1) {
        muted := 0
...
VolumeDown() {
    global muted, volume, volumeBeforeMuted, volumeStep, socket, oscAddress
    if (muted = 1) {
        muted := 0
        volume := volumeBeforeMuted
    }
...
VolumeMute() {
    global muted, volume, volumeBeforeMuted, socket, oscAddress
    if (muted = 0) {
        muted := 1
...
ShowOSDBar() {
    global volume, volumeMaxValue, osdBarOptions, configFile
